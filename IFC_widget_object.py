...
class IFC_widget_object(QWidget):
    ...
    def __init__(self):
        ...
        #버튼
        self.btn1 = QPushButton("공사에 객체 추가")
        ...
        self.object_tree.setHeaderLabels(["객체", "참고"])
        self.object_tree.setFont(font)
        self.object_tree.setColumnWidth(0,240)
    ...
    def add_objects(self, filename):
        ...
        try:
            for item in ifc_file.by_type(self.root_class): #            for item in ifc_file.by_type(self.root_class):
    ...
    def add_object_in_tree(self, ifc_object, parent_item):
        ...
        # QTreeWidgetItem에 재료 이름들과 객체 이름을 순서를 바꿔서 전달
        ...
        if self.follow_decomposition:
            ...
            if ifc_object.is_a('IfcWall'):
                wall_type = ifcopenshell.util.element.get_type(ifc_object)
                ...
                for layer_sequence_number, layer in enumerate(layer_sequence):
                    ...
                    self.add_object_in_tree_material(layer, tree_item)
           

    def add_object_in_tree_material(self, layer, parent_item):
        
        my_name = layer.Material.Name
        # QTreeWidgetItem에 재료 이름들과 객체 이름을 순서를 바꿔서 전달
        tree_item = QTreeWidgetItem([my_name,layer.is_a()])
        # tree_item = QTreeWidgetItem([my_name, ifc_object.is_a()])
        parent_item.addChild(tree_item)
        tree_item.setData(0, Qt.UserRole, layer)
        tree_item.setToolTip(0, entity_summary(layer))
